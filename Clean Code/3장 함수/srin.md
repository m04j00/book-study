...
함수명만 보고 사용한 함수에서 생각치도 못한 부수적 기능을 수행함으로써 원하는 결과가 나오지 않을 수도 있다.
...
> 객체 지향이 나오기 전에는 출력 인수가 불가피한 경우도 있었다. 하지만 객체 지향 언어에서는 출력 인수를 사용할 필요가 거의 없다.
> 

this를 사용하면 출력인수를 사용할 일이 거의 없다.  
...
명령과 조회를 분리하지 않으면 사용자로 하여금 무조건 선언문을 보아야만 이해할 수 있는 상황을 야기 시킨다. 
...
> 오류 코드를 반환하면 호출자는 오류 코드를 곧바로 처리해야 하다는 문제에 부딪힌다.
> 
...
> 오류 코드를 반환한다는 이야기는, 클래스든 열거형 변수든, 어디선가 오류 코드를 정의한다는 뜻이다. ~~(이것이 의존성 자석이다.)~~
> 
...
> 데이크스트라는 모든 함수와 함수 내 모든 블록에 입구와 출구가 하나만 존재하야 한다고 말했다.
> 

> 루프 안에서 break나 continue를 사용해선 안 되며, goto는 **절대로 절대로** 안 된다.
> 

> 구조적 프로그래밍의 목표과 규율은 공감하지만 함수가 작다면 위 규칙은 별 이익을 제공하지 못한다. 함수가 아주 클 때만 상당한 이익을 제공한다.
> 
...
여느 글짓기 처럼 처음에는 뒤죽박죽이지만 이를 다듬고, 이름을 바꾸고, 중복을 없애는 등의 작업을 통해 최종적으로 이 장에서 설명한 규칙을 따르는 함수가 만들어진다고 말한다.
...
> 함수는 그 언어(도메인 특화 언어:DSL)에서 동사며, 클래스는 명사다.
> 
...
약 2주동안 길다면 길고 짧다면 짧은 [함수]라는 주제를 다룬 한 장이 끝났다. 프로그램을 개발하며 가장 많이 하는 것 중 하나인 함수 만들기는 결코 쉬운 일이 아니였음을 깨달았다. 단순히 [함수]라는 단어 자체로써만 생각했던 개념이 입체적으로 그려지는 시간이었다고 생각한다. 이와 함께 나는 이 저자가 생각하는 [함수]에 부합하는 [함수]를 단 한번이라도 만들어 본적이 있나? 라는 의문을 품게 되었다. 이 저자는 코드 하나하나를 매우 소중히 다루는 느낌이었다. 단순히 ‘코드’가 아닌 정말 그 코드를 이용하여 이야기를 만들어내는 프로 작가의 모습이 보였다. 이책에서는 아래와 같이 말한다.

> 소프트웨어를 짜는 행위는 여느 글짓기와 비슷하다.
> 

머리를 한대 맞은 듯한 문구였다. 생각해보면, 소프트웨어를 짜는 행위는 저자가 말하듯 글짓기와 같다. 하지만 그 프로 작가들과 나의 차이가 있다면 자신의 글에 대한 애정이 아니였을까 생각한다. 그래서 조금은 나도 내가 쓰는 코드라는 글에 애정을 가지고 저자가 이야기 하듯 함수를 다듬고, 더 나은 코드(글)를 쓰기 위해 노력해야겠다는 생각이 크게 들었다.
